...
방향의 기준은 `01-project-direction.md`, 선택 이유는 `03-decisions.md`, 실제 진행률은 `04-status-and-handoff.md`를 따른다. 아래 상세 구현은 기본 제안이며, 아직 생성한 코드·DB·계정·배포를 의미하지 않는다.
...
| S3 맥락의 연속성 | 다음 날 작업 이어가기 | 진행 중인 일, 결정, 다음 행동, Context | 새 대화에서 작업 재개·완료 반영 |
| S4 자동 기억과 검색 | 중요한 내용 선별 | 후보 추출, 검증, 작업 복구, hybrid 검색 | 고정 평가셋 비교, 중복·정정·삭제 회귀 확인 |
...
| S3-01 | work_items/decisions | S2 | 진행·완료·결정·다음 행동 저장 |
| S3-02 | 작업 선택과 Context 구성 | S3-01 | 모호한 작업 참조 확인 가능 |
| S3-03 | 연속성 평가와 실사용 | S3-02 | 아래 시나리오 수행 결과 기록 |

S4 이후는 해당 단계 진입 전에 Task를 상세화한다. 현재는 대규모 완성 스키마를 미리 만들지 않는다.
