...
버전: 0.1 · 작성일: 2026-09-15 · 최종 갱신: 2026-09-18 · 상태: S0~S3과 S4-0 구현 완료, S4-A 이후는 계획

방향의 기준은 `01-project-direction.md`, 선택 이유는 `03-decisions.md`, 실제 진행률은 `04-status-and-handoff.md`를 따른다. **S3 이후는 문서 두 개가 나눠 갖는다.** `06-product-definition.md`가 무엇을 왜 만드는지(제품 목표·발전 순서·우선순위 기준)를, `05-conversations-topics-and-map.md`가 대화·주제·지도의 상세 설계를 갖는다. 이 문서의 S3·S4 항목과 어긋나면 06을 우선하고 그 이유를 03에 남긴다. S0~S3은 실제로 구현해 배포했다. 아래 상세 구현 중 S4 이후 항목은 기본 제안이며 아직 만들지 않은 것이다.
...
`06-product-definition.md` 12절은 제품 능력의 발전 순서를 A~E로 설명한다. 이 문서의 S번호를 대체하는 것이 아니라 **왜 그 순서인지**를 설명한다. 대응은 다음과 같다.

| 06의 단계 | 이 문서의 범위 | 현재 |
| --- | --- | --- |
| A 알고 이어가기 | S2 기억, S3 작업 연속성, S4 자동 추출·검색 | **미완** — 초기 정보 가져오기는 S4-0으로 끝냈고, 자동 추출과 여러 대화 검색이 남았다 |
| B 맡긴 일 챙기기 | S3-C 확장, 마감·상기 | 부분 — 일과 다음 행동은 있고 상기가 없다 |
| C 미리 준비하기 | S6 선제 브리핑 | 미착수 |
| D 승인받아 행동하기 | S7 제한된 행동 | 미착수 |
| E 제한적으로 자동화하기 | S7 이후 | 미착수 |

**A가 끝나지 않았다.** S3까지 구현했지만 06의 A단계 완료 기준("배경을 반복하지 않고 이전 일을 이어갈 수 있는가")을 아직 만족하지 못한다. 실사용에서 새 대화가 사용자를 모르고 이전 대화를 찾지 못하는 것이 확인됐다(2026-09-17).

S4-0(초기 맥락 가져오기)으로 기억 0건 문제의 출발점은 해소했다(2026-09-18). 남은 것은 대화에서 기억이 저절로 쌓이는 경로(S4-A)와 여러 대화에 걸친 검색(S4-B)이다.
...
S4는 06의 A단계를 끝내는 작업이다. 우선순위는 06의 13절 기준으로 정했고 현재 순서는 다음과 같다.

| ID | 작업 | 근거 |
| --- | --- | --- |
| ~~S4-0~~ | ~~초기 사용자 맥락 가져오기~~ **완료(2026-09-18)** | 06 8절. 기억 0건 상태를 한 번에 채운다. 후보·근거 대조·민감 정보 처리는 S4-A가 그대로 물려받는다 |
| S4-A | 대화에서 기억 자동 추출 | 06 A단계. 수동 등록에 의존하지 않는다 |
| S4-B | 여러 대화에 걸친 기억·대화 검색 | 06 A단계의 남은 항목. 현재는 같은 대화만 읽는다 |
| S4-C | 자동 주제 발견 | 05 5절 |
| S4-D | 주제 관계도 | 05 7.1절 |

임베딩 기반 대화 지도는 그 뒤의 실험이다. 해당 단계 진입 전에 Task를 상세화하며, 지금 대규모 완성 스키마를 미리 만들지 않는다.
...
| E11 | 가져온 기억만 있을 때 배경 질문 | 아는 것을 말함. 모른다고만 하지 않음 |
| E12 | 가져온 목표 질문 | 목표를 말하되 언제 기준인지 밝힘 |
| E13 | 방금 등록한 일 질문 | 저장된 일을 말하고 오래됐다고 하지 않음 |
